...
### Technical Info
 ...
 - [Setup Git](#setup-git)
 - [Setup StartingInventory](#setup-startinginventory)
 - [Compile StartingInventory](#compile-startinginventory)
 - [Updating Your Repository](#updating-your-repository)

#### Setup Java
The Java JDK is used to compile StartingInventory.
...
#### Setup Gradle
Gradle is used to execute the various build tasks when compiling StartingInventory.
...
#### Setup Git
Git is used to clone StartingInventory and update your local copy.
...
***
    mcdev
    \-StartingInventory
        \-StartingInventory's files (should have build.gradle)
***

#### Compile StartingInventory
1. Execute `gradle setupDecompWorkspace`. This sets up Forge and downloads the necessary libraries to build StartingInventory. This might take some time, be patient.
    * You will generally only have to do this once until the Forge version in `build.properties` changes.
2. Execute `gradle build`. If you did everything right, `BUILD SUCCESSFUL` will be displayed after it finishes. This should be relatively quick.
    * If you see `BUILD FAILED`, check the error output (it should be right around `BUILD FAILED`), fix everything (if possible), and try again.
3. Go to `mcdev\StartingInventory\build\libs`.
    * You should see a `.jar` file named `[#.#.#]StartingInventory-<version>.jar`.
